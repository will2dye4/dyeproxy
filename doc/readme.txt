...
William Dye, at <williamdye@gatech.edu>.
...
The request verb must be 'GET', and the hostname in the request must be valid. If all of
...
port number after the hostname in the request is not supported.

While it is running, dyeproxy waits for a connection from a client, waits for a request from
the client, parses and forwards the request, returns the response, and then begins
waiting for a new connection to repeat the cycle. When receiving a response from an origin 
server, the socket timeout it set to 15 seconds to prevent one slow connection from tying 
up the whole proxy. (More motivation for allowing parallel connections?)
